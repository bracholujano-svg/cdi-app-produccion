...
const EtpCopilotForm = ({ onSave, initialColorRef }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden min-h-[800px]">
        ...
        <div className="p-6 relative z-10 flex flex-col gap-4 flex-grow">
          ...
          <div>
            ...
            <div className="relative">
              <input type="text" value={colorRef} onChange={(e) => setColorRef(e.target.value)} placeholder={colorSystem === 'RAL' ? "Ej. 1023" : "Ej. Nombre o Código"} className="w-full pl-3 pr-10 py-2.5 rounded-md bg-slate-800 border border-slate-700 text-white focus:ring-2 focus:ring-blue-500 outline-none text-sm uppercase placeholder-slate-500 font-bold"/>
          ...
          <button onClick={handleAiAnalysis} disabled={isLoading} className="w-full py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 text-white rounded-lg text-sm font-bold shadow-md transition-all flex items-center justify-center gap-2 mt-2">
            <Brain size={16} className={isLoading ? "animate-spin" : ""} /> Analizar e Inyectar Fórmula
          </button>
      ...
      {/* PANEL DERECHO: VISTA PREVIA ETP */}
        ...
        <header className="bg-white p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 sticky top-0 z-20 shadow-sm">
            <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                <div className="flex-grow">
                    ...
                    <input type="text" name="nombreComercial" value={formData.nombreComercial} onChange={handleChange} placeholder="Nombre Comercial..." className="text-xl font-bold bg-transparent border-b border-transparent hover:border-slate-300 focus:border-blue-500 outline-none w-full text-slate-900 transition-colors"/>
                    <input type="text" name="codigoInterno" value={formData.codigoInterno} onChange={handleChange} placeholder="CÓDIGO INTERNO" className="text-xs text-slate-500 bg-transparent border-none outline-none mt-0.5 uppercase font-medium w-full"/>
                </div>
            </div>
