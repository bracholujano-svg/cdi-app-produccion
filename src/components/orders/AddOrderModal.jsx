            ...
            <div className="overflow-y-auto p-5 md:p-8 custom-scrollbar">
                <div className="bg-[var(--color-surface)] p-5 rounded-[1.5rem] border border-[var(--color-border)] shadow-inner mb-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <input value={excelSearchArticulo} onChange={e=>setExcelSearchArticulo(e.target.value)} placeholder="ÚLT. DÍGITOS ARTÍCULO" className="flex-1 p-3.5 bg-white text-black rounded-xl font-black text-xs md:text-sm lg:text-base outline-none focus:ring-2 focus:ring-[var(--color-primary)] uppercase placeholder:text-black/30" />
                        <button type="button" onClick={doExcelSearch} disabled={excelSearchLoading} className="bg-[var(--color-primary)] text-[var(--color-surface)] px-6 py-3.5 rounded-xl font-black text-xs md:text-sm lg:text-base uppercase shadow-sm border border-[var(--color-border)] transition-colors duration-200   hover:brightness-125 active:scale-95 disabled:opacity-50 shrink-0">
                    ...
                    {showSearchSelector && searchResults.length > 0 && (
                      <div className="mt-4 animate-in slide-in-from-top-2">
                        <button 
                          ...
                          className="w-full mb-3 p-3 bg-[var(--color-primary)] text-[var(--color-surface)] rounded-xl font-black uppercase text-xs md:text-sm lg:text-base hover:brightness-110 active:scale-95 transition-colors flex justify-center items-center gap-2"
                        >
                          <Package size={18} />
                ...
                <form id="nuevoRegistroForm" onSubmit={createOrder} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="md:col-span-2 space-y-1 mt-2">
                        ...
                        <select name="areaRecibe" className="w-full p-4 bg-[var(--color-primary)] text-[var(--color-surface)] rounded-xl font-black uppercase text-xs md:text-sm lg:text-base border  outline-none shadow-sm cursor-pointer focus:ring-2 focus:ring-white">
