          ...
          <div className="w-full max-w-[95vw] theme-bg-main h-[90vh] sm:h-[95vh] rounded-[2rem] flex flex-col border theme-border shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="p-5 theme-bg-header border-b theme-border flex justify-between items-center shrink-0">
              <div className="flex-1 min-w-0 pr-4">
                 <h2 className="text-xl font-black theme-text-main truncate flex items-center gap-2">ORDEN: {activeGroupObj.pedidoNum} <span className="text-sm bg-[var(--color-primary)]/10 px-2 py-0.5 rounded-lg theme-text-main font-bold tracking-widest border border-[var(--color-primary)]/20">({activeGroupObj.products.length} ÍTEMS)</span></h2>
              ...
              <button type="button" onClick={() => { setSelectedGroupPedido(null); setShowDashboardModal(true); }} className="px-3 py-2 bg-[var(--color-primary)]/10 rounded-xl hover:bg-[var(--color-primary)]/20 transition-colors theme-text-main text-sm font-bold mr-2">⬅ Panel IA</button>
              <button type="button" onClick={() => setSelectedGroupPedido(null)} className="p-2.5 bg-black/10 rounded-xl hover:bg-black/20 transition-colors theme-text-main shrink-0">✕</button>
            </div>

            <div className="p-4 border-b theme-border bg-black/5 shrink-0 flex items-center gap-3">
                <button type="button" onClick={selectAll} className="p-3 bg-[var(--color-primary)]/10 theme-text-main rounded-xl hover:bg-[var(--color-primary)]/20 transition-colors shrink-0 border border-[var(--color-primary)]/20 flex items-center justify-center" title="Seleccionar/Deseleccionar Todos">
                  ...
                  <div key={p.id} onClick={() => setSelectedOrder(p)} className={`theme-bg-card p-4 rounded-2xl border-[2px] cursor-pointer transition-colors active:scale-95 bg-[var(--color-surface)] relative flex flex-col justify-between ${
                    ...
                    <div>
                        <button type="button" className="absolute top-3 right-3 p-1 rounded-md hover:bg-black/10 transition-colors" onClick={(e) => toggleSelection(e, p)}>
                            {selectedBulkOrders.some(o => o.id === p.id) ? <CheckSquare className="theme-text-main" size={"1.4em"} /> : <Square className="theme-text-main/30" size={"1.4em"} />}
                        </button>
                        ...
                        <h4 className="font-black text-base lg:text-lg uppercase leading-tight theme-text-main">{p.nombre}</h4>
                    </div>

                    <div className="mt-4 p-2 bg-[var(--color-base)] rounded-xl border border-[var(--color-border)] flex flex-col gap-1.5">
                      <p className="text-base lg:text-lg font-black theme-text-main uppercase flex items-center gap-1 truncate" title={p.activeAreas && p.activeAreas.length > 1 ? `ÁREAS ACTUALES: ${p.activeAreas.join(', ')}` : p.areaActual}>
                      ...
                      {isPartial && lastPartial && (
                        ...
                        >
                            <span 
                                style={{ color: '#7c2d12' }}
                                className="text-sm md:text-base font-black uppercase flex items-center gap-1.5"
                            >
                                <Package size="1.2em" style={{ color: '#c2410c' }} /> ENTREGA PARCIAL
                            </span>
                            <span 
                                style={{ color: '#9a3412' }}
                    ...
                    <div className="mt-3">
                        <button 
                            ...
                            className="w-full bg-[var(--color-primary)]/10 theme-text-main hover:bg-[var(--color-primary)]/20 py-2 rounded-xl flex items-center justify-center gap-2 font-black text-sm md:text-base uppercase transition-colors"
                        >
              ...
              {totalPages > 1 && (
                <div className="mt-6 flex justify-center items-center gap-4 pb-4">
                  <button 
                    ...
                    className="px-6 py-3 border theme-border theme-bg-card theme-text-main font-black uppercase text-base lg:text-lg rounded-xl disabled:opacity-50 hover:bg-[var(--color-primary)] hover:text-[var(--color-surface)] transition-colors active:scale-95 shadow-sm"
                  >
                    Anterior
                  </button>
                  <span className="font-bold text-base lg:text-lg theme-text-main px-2">
