    ...
    <>
      ...
      className={`fixed top-0 left-0 h-full w-[110px] md:w-[130px] bg-[var(--color-base)] z-[80] border-r border-[var(--color-border)] shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col items-center py-6 gap-6 overflow-y-auto custom-scrollbar ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-12 flex flex-col gap-4 w-full px-4 mb-10">
            ...
            <button type="button" onClick={() => { setIsSidebarOpen(false); setShowDossierModal(true); }} className="bg-[var(--color-surface)] aspect-square w-full rounded-2xl flex flex-col items-center justify-center gap-2 font-black text-sm md:text-base uppercase shadow-lg text-purple-400 border border-purple-500/30 transition-colors duration-200 hover:text-white hover:bg-purple-600 hover:border-purple-600 hover:-translate-y-1">
            ...
            <div className="w-full flex flex-col gap-2 relative group" ref={themeDropdownRef}>
              <button type="button" onClick={() => setIsThemeOpen(!isThemeOpen)} className={`bg-[var(--color-surface)] aspect-square w-full rounded-2xl flex flex-col items-center justify-center gap-2 font-black text-sm md:text-base uppercase shadow-lg text-pink-400 border transition-colors duration-200 hover:text-white hover:bg-pink-600 hover:border-pink-600 hover:-translate-y-1 ${isThemeOpen ? 'bg-pink-600/20 border-pink-500' : 'border-pink-500/30'}`}>
