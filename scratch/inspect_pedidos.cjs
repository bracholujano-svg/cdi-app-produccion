...
async function inspect() {
    ...
    const invRes = await fetch(url + "produccion_pedidos?select=*&select=Id%20Referencia&limit=1", {
        headers: { "apikey": key, "Authorization": "Bearer " + key }
    });
    const invData = await invRes.json();
    console.log("produccion_pedidos sample:", JSON.stringify(invData, null, 2));
