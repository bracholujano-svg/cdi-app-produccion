...
// 5. Arreglar Entregas: Remover el input de "recibidoPor"
content = content.replace(
    /(<div className="grid grid-cols-2 gap-2">\s*<input id="entregadoPor"[^>]+>\s*)<input id="recibidoPor"[^>]+>\s*(<\/div>)/,
...
// Reemplazar la lógica de onClick del botón (usando string estático para evitar problemas de regex)
const buttonOldString = `<button type="button" onClick={()=>{
...
const buttonNewString = `<button type="button" onClick={()=>{
                            const en = document.getElementById('entregadoPor').value.trim().toUpperCase();
...
content = content.replace(buttonOldString, buttonNewString);
...
console.log('OrderDetailsModal parcheado con éxito v3!');
