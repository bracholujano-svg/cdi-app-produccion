...
// Regex para capturar todo el bloque de renderizado del modal de alertas de inventario
const oldModalContentRegex = /\{\(\) => \{\s*const isModalAlert = activeAlertMaterials\.some\(m => m\.faltante > 0\);[\s\S]*?\}\(\)\s*\}/;
                  ...
                  <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-[#0B0F19]">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                  ...
                                  {withoutStock.map((mat, i) => (
                                      <div key={i} className="p-4 rounded-xl border border-orange-500/30 bg-orange-500/5 hover:bg-orange-500/10 transition-colors flex flex-col gap-2 relative overflow-hidden group">
                                          ...
                                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2 border-t border-orange-500/20 pt-3">
                                              <div className="flex flex-col bg-black/20 p-2 rounded-lg"><span className="text-[9px] font-black text-slate-400 uppercase">Solicitada</span><span className="text-xs font-black text-slate-200">{mat.requerida}</span></div>
                                              <div className="flex flex-col bg-black/20 p-2 rounded-lg"><span className="text-[9px] font-black text-slate-400 uppercase">Asignada</span><span className="text-xs font-black text-slate-200">{mat.asignada}</span></div>
                                              <div className="flex flex-col bg-red-500/10 border border-red-500/20 p-2 rounded-lg"><span className="text-[9px] font-black text-red-400 uppercase leading-tight">Faltante Comprar</span><span className="text-xs font-black text-red-500">{mat.faltante}</span></div>
                                              <div className="flex flex-col bg-black/20 p-2 rounded-lg"><span className="text-[9px] font-black text-slate-400 uppercase leading-tight">Stock Remanente</span><span className="text-xs font-black text-slate-400">{mat.stockRestanteGlobal}</span></div>
                                          </div>
                                      </div>
                                  ))}
                          ...
                          <div className="flex flex-col gap-3">
                              ...
                              <div className="space-y-3">
                                  {withStock.map((mat, i) => (
                                      <div key={i} className="p-4 rounded-xl border border-green-500/30 bg-green-500/5 hover:bg-green-500/10 transition-colors flex flex-col gap-2">
                                          ...
                                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2 border-t border-green-500/20 pt-3">
                                              <div className="flex flex-col bg-black/20 p-2 rounded-lg"><span className="text-[9px] font-black text-slate-400 uppercase">Solicitada</span><span className="text-xs font-black text-slate-200">{mat.requerida}</span></div>
                                              <div className="flex flex-col bg-black/20 p-2 rounded-lg"><span className="text-[9px] font-black text-slate-400 uppercase">Asignada</span><span className="text-xs font-black text-slate-200">{mat.asignada}</span></div>
                                              <div className="flex flex-col bg-green-500/10 border border-green-500/20 p-2 rounded-lg"><span className="text-[9px] font-black text-green-400 uppercase leading-tight">Faltante Comprar</span><span className="text-xs font-black text-green-500">0</span></div>
                                              <div className="flex flex-col bg-black/20 p-2 rounded-lg"><span className="text-[9px] font-black text-slate-400 uppercase leading-tight">Stock Remanente</span><span className="text-xs font-black text-slate-400">{mat.stockRestanteGlobal}</span></div>
                                          </div>
                                      </div>
                                  ))}
...
if (oldModalContentRegex.test(content)) {
    content = content.replace(oldModalContentRegex, newModalContent);
    fs.writeFileSync(filePath, content, 'utf-8');
    console.log('App.jsx modal rediseñado con éxito!');
} else {
    console.log('No se encontró el modal en App.jsx, usando fallback regex...');
    // Fallback: tratar de encontrar el bloque manual
    const fallbackRegex = /\{\(\)\s*=>\s*\{\s*const isModalAlert\s*=\s*activeAlertMaterials\.some[\s\S]*?\}\(\)\s*\}/;
    if (fallbackRegex.test(content)) {
        content = content.replace(fallbackRegex, newModalContent);
        fs.writeFileSync(filePath, content, 'utf-8');
        console.log('App.jsx modal rediseñado usando fallback regex!');
    } else {
