...
const EtpCopilotForm = ({ onSave }) => {
  ...
  // Estado para la tabla de formulación simulada
  ...
  // Función para guardar procedimiento personalizado como plantilla reutilizable
  ...
  const handleAiAnalysis = () => {
    ...
    setTimeout(() => {
      ...
      if (input.includes('RAL 1') || input.includes('RAL 2') || input.includes('RAL 3') || input.includes('PANTONE 1')) {
        diag = `> Match: ${input}\n> Familia: Cálida (Pigmentos orgánicos)\n> ALERTA: Baja opacidad detectada.\n> Directriz ETP: Obligatorio aislar con PU Blanco puro para garantizar L*.`;
        updates = {
          ...updates,
          fondoRequired: 'Poliuretano Blanco Alta Cubriente (Ref: PU-BL-01)',
        ...
        setFormula([
            { id: 1, componente: 'Base Poliuretano Transp.', peso: '750.0', color: 'bg-slate-200' },
            { id: 2, componente: 'Tinte Amarillo Cromo (T-22)', peso: '210.0', color: 'bg-yellow-400' },
            { id: 3, componente: 'Tinte Naranja (T-15)', peso: '40.0', color: 'bg-orange-500' }
        ]);
      } else if (input.includes('RAL 4') || input.includes('RAL 5') || input.includes('NCS S 8')) {
        diag = `> Match: ${input}\n> L* estimado: < 35 (Profundo)\n> Directriz ETP: Requiere Primer Gris Oscuro. Usar blanco causará sobreconsumo.`;
        updates = {
          ...updates,
          fondoRequired: 'Primer Gris Oscuro o Negro (Ref: PR-GR-02)',
          requiereFondoBlancoPuro: false,
          manos: '2 Manos Cruzadas',
         ...
         diag = `> Match: ${input}\n> Comportamiento estándar detectado.\n> Directriz ETP: Compatible con preparación universal.`;
         updates = {
           ...updates,
           fondoRequired: 'Base Blanca o Gris Claro Universal',
           requiereFondoBlancoPuro: false,
      ...
      <aside className="w-full lg:w-[320px] bg-slate-900 border-r border-slate-800 flex flex-col relative overflow-hidden flex-shrink-0">
        ...
        <div className="p-6 relative z-10 border-b border-slate-800">
          ...
          <div className="mt-4 bg-slate-800 rounded-lg p-2.5 border border-slate-700 flex items-center gap-2">
            ...
            <p className="text-[10px] text-slate-300">Conectado a DB Global</p>
          </div>
        </div>

        <div className="p-6 relative z-10 flex flex-col gap-4 flex-grow">
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Sistema de Color</label>
            <select 
              value={colorSystem}
              onChange={(e) => setColorSystem(e.target.value)}
              className="w-full px-3 py-2.5 rounded-md bg-slate-800 border border-slate-700 text-white focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium"
            >
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Referencia del Color</label>
            <div className="relative">
              <input 
                type="text" 
                value={colorRef}
                onChange={(e) => setColorRef(e.target.value)}
                placeholder={colorSystem === 'RAL' ? "Ej. 1023" : colorSystem === 'PANTONE' ? "Ej. 185 C" : "Ej. Nombre o Código"} 
                className="w-full pl-3 pr-10 py-2.5 rounded-md bg-slate-800 border border-slate-700 text-white focus:ring-2 focus:ring-blue-500 outline-none text-sm uppercase placeholder-slate-500 font-bold"
              />
          ...
          <div>
            ...
            >
              {Array.from({ length: 20 }, (_, i) => (i + 1) * 5).map(val => {
                let label = `${val}%`;
                if (val <= 10) label += ' (Mate)';
                else if (val === 40) label += ' (Satinado)';
                else if (val >= 90) label += ' (Alto Brillo)';
          ...
          <button 
            ...
            disabled={isLoading}
            className="w-full py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 text-white rounded-lg text-sm font-bold shadow-md transition-all flex items-center justify-center gap-2 mt-2"
          >
            {isLoading ? <Brain size={16} className="animate-spin" /> : <Brain size={16} />}
            Analizar e Inyectar Fórmula
          </button>
      ...
      <div className="w-full flex flex-col bg-slate-50 relative h-[800px] overflow-y-auto custom-scroll">
        ...
        <header className="bg-white p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 sticky top-0 z-20 shadow-sm">
            <div className="flex items-center gap-4 w-full md:w-auto">
                <div className="w-14 h-14 rounded-md shadow-inner border border-slate-200 flex-shrink-0" style={{backgroundColor: formula.length > 0 ? (colorRef.includes('RAL 1') ? '#facc15' : '#1e3a8a') : '#e2e8f0'}}></div>
                <div className="flex-grow">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        {aiDiagnosis && <span className="text-blue-600 text-[10px] font-bold flex items-center gap-1"><CheckCircle2 size={12}/> IA Generado</span>}
                    </div>
                    <input 
                        type="text" name="nombreComercial" value={formData.nombreComercial} onChange={handleChange} 
                        placeholder="Nombre Comercial..." 
                        className="text-xl font-bold bg-transparent border-b border-transparent hover:border-slate-300 focus:border-blue-500 outline-none w-full text-slate-900 transition-colors"
                    />
                    <input 
                        type="text" name="codigoInterno" value={formData.codigoInterno} onChange={handleChange} 
                        placeholder="CÓDIGO INTERNO" 
                        className="text-xs text-slate-500 bg-transparent border-none outline-none mt-0.5 uppercase font-medium w-full"
                    />
                </div>
            </div>
            
            {/* DELTA E MUY VISIBLE Y DESTACADO */}
            <div className="bg-gradient-to-r from-blue-900 to-indigo-900 text-white p-3 rounded-xl shadow-md border border-blue-700/50 flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
                <div className="text-left md:text-right">
                    ...
                    <span className="text-xs font-semibold text-emerald-300">Control de Color Calibrado</span>
                </div>
                <div className="bg-blue-800 px-3 py-1.5 rounded-lg border border-blue-600 shadow-inner flex items-center gap-1.5">
                    ...
                    <input 
                        type="text" name="deltaE" value={formData.deltaE} onChange={handleChange} 
                        className="text-sm font-black text-white bg-transparent outline-none w-24 text-right tracking-tight cursor-pointer"
                        title="Haga clic para editar tolerancia Delta E"
                    />
                </div>
            </div>
        </header>
        ...
        <form onSubmit={handleSubmit} className="flex-grow flex flex-col">
          ...
          <div className="px-6 pt-6">
            <div className={`p-5 rounded-r-lg shadow-sm border-l-4 transition-all duration-500 flex items-start gap-4 ${formData.fondoRequired ? 'bg-amber-50 border-amber-500' : 'bg-slate-100 border-slate-300'}`}>
                ...
                <div className="flex-grow">
                    ...
                    <p className={`text-xs mt-1 mb-3 ${formData.fondoRequired ? 'text-amber-800' : 'text-slate-500'}`}>
                        Para lograr el tono exacto, este color <strong>DEBE</strong> aplicarse exclusivamente sobre:
                    </p>
                    
                    <div className="relative">
                        <Layers size={16} className="absolute left-3 top-2.5 text-slate-400" />
                        <input 
                            type="text" name="fondoRequired" value={formData.fondoRequired} onChange={handleChange} 
                            placeholder="Esperando análisis de IA..."
                            className={`w-full pl-9 pr-3 py-2 rounded shadow-sm border text-sm font-bold outline-none transition-colors ${formData.fondoRequired ? 'bg-white border-amber-300 text-slate-800' : 'bg-slate-50 border-slate-200 text-slate-500'}`}
                        />
                    </div>
          ...
          <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              {/* COLUMNA IZQUIERDA: FORMULA Y SELECTORES MULTIPLES */}
              <div className="lg:col-span-5 space-y-6">
                  <div>
                          ...
                          <table className="w-full text-left text-sm">
                              ...
                              <tbody className="divide-y divide-slate-100 text-slate-700">
                                  {formula.map((item, index) => (
                                      <tr key={item.id} className={highlightFields ? 'bg-blue-50/30' : ''}>
                                          <td className="py-3 px-4 flex items-center gap-2 text-xs font-medium">
                                              <div className={`w-3 h-3 rounded-full border border-slate-200 shadow-sm ${item.color}`}></div>
                                              {item.componente}
                                          </td>
                                  ...
                                  <tr className="bg-slate-50 border-t-2 border-slate-200">
                                      <td className="py-3 px-4 text-xs font-bold text-slate-800 uppercase tracking-wide">Peso Total Entonador</td>
                      ...
                      {/* SELECTORES MÚLTIPLES PARA CATALIZADOR Y DISOLVENTE */}
                      <div className="mt-4 grid grid-cols-1 gap-3">
                          <div className="bg-white border border-slate-200 p-3 rounded-lg shadow-sm">
                              <div className="flex justify-between items-center mb-1">
                                  <label className="text-[10px] font-bold text-slate-500 uppercase">Selector de Catalizador</label>
                                  <span className="text-[10px] text-blue-600 font-semibold">Multi-opción</span>
                              </div>
                              <select 
                                  name="catalizador" 
                                  value={formData.catalizador} 
                                  onChange={handleChange}
                                  className="w-full bg-slate-50 border border-slate-200 p-2 rounded text-xs font-bold text-slate-800 outline-none focus:border-blue-500"
                              >
                                  {catalystOptions.map((opt, idx) => (
                          ...
                          <div className="bg-white border border-slate-200 p-3 rounded-lg shadow-sm">
                              <div className="flex justify-between items-center mb-1">
                                  <label className="text-[10px] font-bold text-slate-500 uppercase">Selector de Disolvente (Thinner)</label>
                                  <span className="text-[10px] text-blue-600 font-semibold">Multi-opción</span>
                              </div>
                              <select 
                                  name="disolvente" 
              ...
              {/* COLUMNA DERECHA: PARAMETROS Y HOJA DE RUTA CON PLANTILLAS GUARDADAS */}
              <div className="lg:col-span-7 space-y-8">
                  
                  <div>
                      <h2 className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-4 flex items-center gap-2">
                          <SprayCan size={16} /> 2. Parámetros de Aplicación
                      </h2>
                      
                      <div className="grid grid-cols-3 gap-3">
                          <div className="bg-white border border-slate-200 p-4 rounded-xl text-center shadow-sm">
                              <div className="flex justify-center mb-2"><Droplet className="text-blue-400" size={24} strokeWidth={1.5}/></div>
                              ...
                              <span className="block text-[10px] text-slate-400 mt-1">Copa Ford #4</span>
                          </div>
                          <div className="bg-white border border-slate-200 p-4 rounded-xl text-center shadow-sm">
                              <div className="flex justify-center mb-2"><Gauge className="text-slate-600" size={24} strokeWidth={1.5}/></div>
                              ...
                              <span className="block text-[10px] text-slate-400 mt-1">Pistola HVLP</span>
                          </div>
                          <div className="bg-white border border-slate-200 p-4 rounded-xl text-center shadow-sm">
                              <div className="flex justify-center mb-2"><Crosshair className="text-emerald-500" size={24} strokeWidth={1.5}/></div>
                              ...
                              <div className="flex items-center justify-center gap-1">
                                  ...
                                  <input type="text" name="manos" value={formData.manos} onChange={handleChange} className="w-full text-left font-bold text-slate-800 text-[11px] outline-none bg-transparent whitespace-nowrap overflow-hidden text-ellipsis"/>
                              </div>
                          </div>
                      </div>
                  </div>

                  {/* SECCIÓN 3: HOJA DE RUTA CON SELECTOR DE PLANTILLAS Y BOTÓN GUARDAR */}
                  <div>
                      <div className="flex justify-between items-center mb-4">
                          <h2 className="text-xs font-bold text-blue-600 uppercase tracking-wider flex items-center gap-2">
                              <CheckSquare size={16} /> 3. Hoja de Ruta (Procedimientos Reutilizables)
                          </h2>
                          <span className="text-[10px] bg-blue-50 text-blue-700 font-semibold px-2 py-0.5 rounded border border-blue-200">
                              No requiere copiar y pegar
                          </span>
                      </div>
                          ...
                          <div className="relative pl-6">
                              ...
                              <div className="flex justify-between items-center mb-1">
                                  ...
                                  <div className="flex items-center gap-2">
                                      <select 
                                          className="text-[11px] bg-white border border-slate-300 rounded px-2 py-0.5 text-slate-700 outline-none font-medium"
                                          onChange={(e) => { if(e.target.value) setFormData({...formData, textoPreparacion: e.target.value}) }}
                                          defaultValue=""
                                      >
                                          <option value="" disabled>Seleccionar plantilla...</option>
                                          {savedProcedures.preparacion.map(tpl => (
                                              <option key={tpl.id} value={tpl.text}>{tpl.title}</option>
                                          ))}
                                      </select>
                                      <button 
                                          type="button" 
                                          onClick={() => handleSaveTemplate('preparacion', formData.textoPreparacion)}
                                          className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-700 px-2 py-0.5 rounded border border-slate-300 flex items-center gap-1 font-semibold"
                              ...
                              <textarea name="textoPreparacion" value={formData.textoPreparacion} onChange={handleChange} rows="2" 
                                  className="w-full text-xs text-slate-600 bg-white border border-slate-200 p-2 rounded resize-none outline-none leading-relaxed focus:border-blue-500"
                              ></textarea>
                          </div>

                          {/* Step 2: Fondo (Highlight) */}
                          <div className={`relative pl-6 p-2 rounded-r-lg border -ml-2 ml-4 transition-colors ${formData.fondoRequired ? 'bg-blue-50/50 border-blue-100' : 'border-transparent'}`}>
                              <span className={`absolute -left-[18px] top-3 w-4 h-4 rounded-full border-2 border-white shadow-sm ${formData.fondoRequired ? 'bg-blue-500 shadow-blue-500/50' : 'bg-slate-300'}`}></span>
                              <div className="flex justify-between items-center mb-1">
                                  <h4 className="font-bold text-xs text-slate-900">Aplicación del Fondo (CRÍTICO)</h4>
                                  <div className="flex items-center gap-2">
                                      <select 
                                          className="text-[11px] bg-white border border-slate-300 rounded px-2 py-0.5 text-slate-700 outline-none font-medium"
                                          onChange={(e) => { if(e.target.value) setFormData({...formData, textoFondo: e.target.value}) }}
                                          defaultValue=""
                                      >
                                          <option value="" disabled>Seleccionar plantilla...</option>
                                          {savedProcedures.fondo.map(tpl => (
                                              <option key={tpl.id} value={tpl.text}>{tpl.title}</option>
                                          ))}
                                      </select>
                                      <button 
                                          type="button" 
                                          onClick={() => handleSaveTemplate('fondo', formData.textoFondo)}
                                          className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-700 px-2 py-0.5 rounded border border-slate-300 flex items-center gap-1 font-semibold"
                                      >
                                          <BookmarkPlus size={12}/> Guardar
                                      </button>
                              ...
                              <textarea name="textoFondo" value={formData.textoFondo} onChange={handleChange} rows="3" placeholder="Instrucciones del fondo..."
                                  className={`w-full text-xs bg-white border border-slate-200 p-2 rounded resize-none outline-none leading-relaxed font-medium ${formData.requiereFondoBlancoPuro ? 'text-blue-900' : 'text-slate-700'} focus:border-blue-500`}
                              ></textarea>
                          </div>
                          ...
                          <div className="relative pl-6">
                              ...
                              <div className="flex justify-between items-center mb-1">
                                  ...
                                  <div className="flex items-center gap-2">
                                      <select 
                                          className="text-[11px] bg-white border border-slate-300 rounded px-2 py-0.5 text-slate-700 outline-none font-medium"
                                          onChange={(e) => { if(e.target.value) setFormData({...formData, textoColor: e.target.value}) }}
                                          defaultValue=""
                                      >
                                          <option value="" disabled>Seleccionar plantilla...</option>
                                          {savedProcedures.color.map(tpl => (
                                              <option key={tpl.id} value={tpl.text}>{tpl.title}</option>
                                          ))}
                                      </select>
                                      <button 
                                          type="button" 
                                          onClick={() => handleSaveTemplate('color', formData.textoColor)}
                                          className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-700 px-2 py-0.5 rounded border border-slate-300 flex items-center gap-1 font-semibold"
                                      >
                                          <BookmarkPlus size={12}/> Guardar
                                      </button>
                              ...
                              <textarea name="textoColor" value={formData.textoColor} onChange={handleChange} rows="2" placeholder="Instrucciones de color..."
                                  className="w-full text-xs text-slate-600 bg-white border border-slate-200 p-2 rounded resize-none outline-none leading-relaxed focus:border-blue-500"
                              ></textarea>
                          </div>
                          ...
                          <div className="relative pl-6">
                              ...
                              <div className="flex justify-between items-center mb-1">
                                  ...
                                  <div className="flex items-center gap-2">
                                      <select 
                                          className="text-[11px] bg-white border border-slate-300 rounded px-2 py-0.5 text-slate-700 outline-none font-medium"
                                          onChange={(e) => { if(e.target.value) setFormData({...formData, textoAcabado: e.target.value}) }}
                                          defaultValue=""
                                      >
                                          <option value="" disabled>Seleccionar plantilla...</option>
                                          {savedProcedures.acabado.map(tpl => (
                                              <option key={tpl.id} value={tpl.text}>{tpl.title}</option>
                                          ))}
                                      </select>
                                      <button 
                                          type="button" 
                                          onClick={() => handleSaveTemplate('acabado', formData.textoAcabado)}
                                          className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-700 px-2 py-0.5 rounded border border-slate-300 flex items-center gap-1 font-semibold"
                                      >
                                          <BookmarkPlus size={12}/> Guardar
                                      </button>
                              ...
                              <textarea name="textoAcabado" value={formData.textoAcabado} onChange={handleChange} rows="2" placeholder="Instrucciones de acabado..."
                                  className="w-full text-xs text-slate-600 bg-white border border-slate-200 p-2 rounded resize-none outline-none leading-relaxed focus:border-blue-500"
                              ></textarea>
                          </div>
                      </div>
          ...
          <div className="bg-white border-t border-slate-200 p-5 mt-auto flex flex-col md:flex-row items-center justify-between gap-4 sticky bottom-0 z-20">
            ...
            <button 
                ...
                className={`px-8 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 w-full md:w-auto ${isConfirmed ? 'bg-slate-800 hover:bg-slate-900 text-white shadow-md' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
            >
