...
// Normalize newlines to avoid CRLF mismatch
code = code.replace(/\r\n/g, '\n');

// 1. Ingredientes
const old_ing = `const ingredientes = Array.isArray(initialData?.ingredientes) 
...
const new_ing = `const [ingredientes, setIngredientes] = useState([]);
...
// 2. handleAiAnalysis
const idx1 = code.indexOf('const handleAiAnalysis = () => {');
const idx2 = code.indexOf('const handleSaveTemplate = (category, text) => {', idx1);

if (idx1 !== -1 && idx2 !== -1) {
    const old_ai = code.substring(idx1, idx2);
    const new_ai = `const undoAi = () => {
    if (previousFormData) {
...
// 5. Tolerancia UI
const idx3 = code.indexOf('<div className="bg-gradient-to-r from-blue-900 to-indigo-900');
const idx4 = code.indexOf('</header>', idx3);

if (idx3 !== -1 && idx4 !== -1) {
    const old_tol = code.substring(idx3, idx4);
    const new_tol = `<div className="bg-gradient-to-r from-blue-900 to-indigo-900 text-white p-3 rounded-xl shadow-md border border-blue-700/50 flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
...
const idx5 = code.indexOf('const handlePrintPDF = async () => {');
const idx6 = code.indexOf('};', code.indexOf('setIsLoading(false);', idx5)) + 2;

if (idx5 !== -1 && idx6 !== -1) {
    const old_print = code.substring(idx5, idx6);
    const new_print = "const handlePrintPDF = () => { window.print(); };";
