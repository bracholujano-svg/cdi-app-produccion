...
const startTag = '{showMaterialsAlertModal && (';
const endTag = '})()\n        )}';
const endTagWin = '})()\r\n        )}';

const startIndex = content.indexOf(startTag);
let endIndex = content.indexOf(endTagWin, startIndex);
let actualEndTag = endTagWin;
if (endIndex === -1) {
...
if (startIndex !== -1 && endIndex !== -1) {
    const oldBlock = content.substring(startIndex, endIndex + actualEndTag.length);
                                  ...
                                  {withoutStock.map((mat, i) => (
                                      <div key={i} className="p-4 rounded-xl border border-orange-500/30 bg-orange-500/5 hover:bg-orange-500/10 transition-colors flex flex-col gap-2 relative overflow-hidden group">
                                          ...
                                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2 border-t border-orange-500/20 pt-3">
                                              <div className="flex flex-col bg-black/20 p-2 rounded-lg"><span className="text-[9px] font-black text-slate-400 uppercase leading-tight">Solicitada</span><span className="text-xs font-black text-slate-200">{mat.requerida.toFixed(2)}</span></div>
                                              <div className="flex flex-col bg-black/20 p-2 rounded-lg"><span className="text-[9px] font-black text-slate-400 uppercase leading-tight">Asignada</span><span className="text-xs font-black text-slate-200">{mat.asignada.toFixed(2)}</span></div>
                                              <div className="flex flex-col bg-red-500/10 border border-red-500/20 p-2 rounded-lg"><span className="text-[9px] font-black text-red-400 uppercase leading-tight">Faltante Comprar</span><span className="text-xs font-black text-red-500">{mat.faltante.toFixed(2)}</span></div>
                                              <div className="flex flex-col bg-black/20 p-2 rounded-lg"><span className="text-[9px] font-black text-slate-400 uppercase leading-tight">Stock Remanente</span><span className="text-xs font-black text-slate-400">{mat.stockRestanteGlobal.toFixed(2)}</span></div>
                                          </div>
                                      </div>
                                  ))}
